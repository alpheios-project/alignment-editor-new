...
<script setup>
import CloseIcon from '@/inline-icons/xclose.svg'
...
const messages = computed(() => {
  const items = Boolean($store.state.notificationUpdated) && $store.state.messages
  if (items && items.length > 0) {
    local.hideTimeout = setTimeout(() => {
        hideMessageBar()
    }, 12000)
  }
  return items
...
const notificationBarClass = computed(() => {
  return {
    'alpheios-invisible': local.hideMessageFlag
  }
})

const notificationClass = (message) => {
  return {
...
const hideMessageBar = () => {
  local.hideMessageFlag = true
  setTimeout(() => {
    messages.value.forEach(message => hideMessage(message))
    local.hideMessageFlag = false
  }, 2000)
}

const hideMessage = (message, clearTimeoutFlag = false) => {
  if (clearTimeoutFlag) {
    clearTimeout(local.hideTimeout)
  }
